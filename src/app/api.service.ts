...
supprimerDocument(id: number): Observable<any> {
  ...
  getVerificationStatus(token: string): Observable<any> {
    let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    if (token) headers = headers.set('Authorization', `Bearer ${token}`);
    return this.http.get(`${this.apiUrl}/auth/verification/status`, { headers });
  }

  requestIdentityVerification(token: string): Observable<any> {
    let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    if (token) headers = headers.set('Authorization', `Bearer ${token}`);
    return this.http.post(`${this.apiUrl}/auth/verification/identity`, {}, { headers });
  }
