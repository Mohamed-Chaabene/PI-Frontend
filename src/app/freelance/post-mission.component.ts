  ...
  genererDescriptionIA(): void {
    ...
    this.freelanceService.generateAiProposal({
      ...
      error: (err: HttpErrorResponse) => {
        const backendDetail = typeof err?.error?.detail === 'string' ? err.error.detail : '';
        this.erreur = backendDetail || 'Impossible de générer la description IA (service indisponible).';
        this.aiLoadingDescription = false;
      }
    });
  }
