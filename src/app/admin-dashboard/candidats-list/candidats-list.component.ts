...
export class CandidatsListComponent implements OnInit {
    ...
    this.apiService.getCandidat(id).subscribe({
      next: (candidate: any) => {
        if (candidate) {
          ...
          this.competenceData.competences = candidate.competences || [];
        }
  ...
  addCompetenceToList(): void {
    const competence = prompt('Entrez une compétence:');
    if (competence?.trim()) {
      this.competenceData.competences.push(competence.trim());
    }
  }

  removeCompetence(index: number): void {
    this.competenceData.competences.splice(index, 1);
  }
  ...
  saveCompetence(): void {
    ...
    const competencePayload = {
      ...
      competences: this.competenceData.competences || []
    };

    this.apiService.updateCandidate(this.selectedCandidatId, competencePayload).subscribe({
      next: () => {
        alert('✅ Compétences et informations sauvegardées avec succès!');
  ...
  getInitials(nom: string, prenom: string): string {
    const first = (nom || 'A').substring(0, 1).toUpperCase();
    const second = (prenom || 'B').substring(0, 1).toUpperCase();
    return first + second;
  }
