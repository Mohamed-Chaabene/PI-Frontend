...
class QuestionTokenizer:
    ...
    def build_vocab(self, texts: list[str]):
        """Construit le vocabulaire depuis les textes du dataset."""
        chars = set()
        for t in texts:
            chars.update(t)
    ...
    def decode(self, ids: list[int]) -> str:
        ...
        return "".join(
            self.id2token.get(i, "") for i in ids if i not in skip
        )
...
class QuestionDataset(Dataset):
    def __init__(self, filepath: str, tokenizer: QuestionTokenizer,
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        src = torch.tensor(
            self.tokenizer.encode(item["input"],  self.input_len),
            dtype=torch.long
        )
        tgt = torch.tensor(
            self.tokenizer.encode(item["output"], self.output_len),
            dtype=torch.long
        )
        return src, tgt