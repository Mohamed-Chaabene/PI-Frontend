...
SEED_PATH = BASE_DIR / "src" / "app" / "Nesrineai" / "data" / "questions.json"
OUT_PATH = BASE_DIR / "src" / "app" / "Nesrineai" / "data" / "questions_expanded.json"
...
def make_augmented_examples(seed_rows: list[dict], per_seed: int = 120) -> list[dict]:
    ...
    for row in seed_rows:
        ...
        for _ in range(per_seed):
            ...
            rows.append({
                "input": input_variant,
                "output": json.dumps(output_variant, ensure_ascii=False),
            })
